...
/**
 * Función actualizada para asignar materia en recursado
 * CORREGIDA: Incluye ciclo_lectivo_id en la inserción
 */
function asignarMateriaRecursado($db, $datos) {
    try {
        $estudianteId = intval($datos['estudiante_id']);
        $materiaCursoId = intval($datos['materia_curso_id']);
        $materiaLiberadaId = intval($datos['materia_liberada_id']);
        $observaciones = trim($datos['observaciones'] ?? '');
        
        // OBTENER CICLO LECTIVO ACTIVO
        $cicloActivo = $db->fetchOne("SELECT * FROM ciclos_lectivos WHERE activo = 1");
        if (!$cicloActivo) {
            return ['type' => 'danger', 'message' => 'No hay ciclo lectivo activo configurado'];
        }
        $cicloLectivoId = $cicloActivo['id'];
        
        // Validación de campos obligatorios
        if ($estudianteId <= 0 || $materiaCursoId <= 0 || $materiaLiberadaId <= 0) {
            return ['type' => 'danger', 'message' => 'Todos los campos son obligatorios'];
        }
        
        // Verificar que el estudiante no esté ya recursando la misma materia
        ...
        // Verificar que la materia liberada no esté ya liberada para otro recursado
        $materiaYaLiberada = $db->fetchOne(
            "SELECT id FROM materias_recursado 
             WHERE estudiante_id = ? AND materia_liberada_id = ? AND ciclo_lectivo_id = ? AND estado = 'activo'",
            [$estudianteId, $materiaLiberadaId, $cicloLectivoId]
        );
        
        if ($materiaYaLiberada) {
            return ['type' => 'danger', 'message' => 'Esta materia ya está liberada para otro recursado'];
        }
        
        // Verificar que no sea la misma materia (no puede recursar la misma que libera)
        $materiaRecursar = $db->fetchOne(
            "SELECT materia_id FROM materias_por_curso WHERE id = ?", 
            [$materiaCursoId]
        );
        $materiaLiberar = $db->fetchOne(
            "SELECT materia_id FROM materias_por_curso WHERE id = ?", 
            [$materiaLiberadaId]
        );
        
        if ($materiaRecursar['materia_id'] == $materiaLiberar['materia_id']) {
            return ['type' => 'danger', 'message' => 'No puede liberar la misma materia que está recursando'];
        }
        
        // Verificar que el estudiante esté matriculado en el curso de la materia liberada
        $matriculaValida = $db->fetchOne(
            "SELECT m.id 
             FROM matriculas m
             JOIN materias_por_curso mp ON m.curso_id = mp.curso_id
             WHERE m.estudiante_id = ? AND mp.id = ? AND m.estado = 'activo'",
            [$estudianteId, $materiaLiberadaId]
        );
        
        if (!$matriculaValida) {
            return ['type' => 'danger', 'message' => 'El estudiante no está matriculado en el curso de la materia que intenta liberar'];
        }
        
        // INSERCIÓN CORREGIDA: Incluir ciclo_lectivo_id
        $db->query(
            "INSERT INTO materias_recursado 
             (estudiante_id, materia_curso_id, materia_liberada_id, ciclo_lectivo_id, fecha_asignacion, observaciones, estado) 
             VALUES (?, ?, ?, ?, date('now'), ?, 'activo')",
            [$estudianteId, $materiaCursoId, $materiaLiberadaId, $cicloLectivoId, $observaciones]
        );
        
        // Obtener nombres de las materias para el mensaje de confirmación
        $materiaRecursarInfo = $db->fetchOne(
        ...
        $materiaLiberadaInfo = $db->fetchOne(
            "SELECT m.nombre, m.codigo 
             FROM materias_por_curso mp 
             JOIN materias m ON mp.materia_id = m.id 
             WHERE mp.id = ?",
        ...
        $mensaje = sprintf(
            'Recursado asignado correctamente. El estudiante cursará "%s (%s)" y no aparecerá en las listas de "%s (%s)".',
            $materiaRecursarInfo['nombre'],
            $materiaRecursarInfo['codigo'],
            $materiaLiberadaInfo['nombre'],
            $materiaLiberadaInfo['codigo']
        );
        
        return ['type' => 'success', 'message' => $mensaje];
        
    } catch (Exception $e) {
        error_log("Error en asignarMateriaRecursado: " . $e->getMessage());
        return ['type' => 'danger', 'message' => 'Error al asignar recursado: ' . $e->getMessage()];
    }
}
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        <button type="button" class="btn btn-warning" onclick="abrirModalRecursado()">  
                            <i class="bi bi-arrow-repeat"></i> Asignar Recursado
                        </button>
                        <a href="gestionar_recursados_mejorado.php" class="btn btn-info">
